...
export const DOM = {
    ...
    initialSearchMessage: document.querySelector("#search-section .initial-message"),
    ...
    initialRecommendationsMessage: document.querySelector("#recommendations-section .initial-message"),

    // Bootstrap modal instance
    bookDetailsModal: new bootstrap.Modal(document.getElementById("bookDetailsModal"), { backdrop: 'static', keyboard: false }),
...
export const showLoadingSpinner = (show) => {
    if (show) {
        ...
        DOM.searchResultsDiv.classList.add("d-none");
        if (DOM.initialSearchMessage) DOM.initialSearchMessage.classList.add("d-none");
    } else {
        DOM.loadingSpinner.classList.remove("d-flex");
        DOM.loadingSpinner.classList.add("d-none");
        DOM.searchResultsDiv.classList.remove("d-none");
    }
};
 ...
 */
export const createBookCard = (book, inUserList = false, listName = null, onAddToList = () => {}, onViewDetails = () => {}) => {
    ...
    const thumbnail = googleThumbnail || openLibraryCover || "https://via.placeholder.com/128x192?text=No+Cover";
    ...
    bookFront.classList.add("book-face", "book-front", "card-body", "d-flex", "flex-column", "align-items-center", "justify-content-between");
    bookFront.innerHTML = `
        ...
        <div class="text-center w-100">
            ...
            ${inUserList ? `<span class="book-status badge bg-secondary mt-2">${listName.charAt(0).toUpperCase() + listName.slice(1)}</span>` : ""}
        </div>
    `;

    // Back Face of the Card (only for search results)
    if (!inUserList) {
        const bookBack = document.createElement("div");
        bookBack.classList.add("book-face", "book-back", "d-flex", "flex-column", "justify-content-center", "gap-2");
        bookBack.innerHTML = `
            ...
            <button class="btn btn-outline-light btn-sm mt-3" data-action="view-details" aria-label="View details for ${title}">Details</button>
        `;
        bookItem.appendChild(bookBack);

        // Event listeners for the back face buttons
        bookBack.querySelectorAll('button').forEach(button => {
 ...
 */
export const displaySearchResults = (books, userBookIds, onAddToList, onViewDetails) => {
    DOM.searchResultsDiv.innerHTML = "";
    if (DOM.initialSearchMessage) DOM.initialSearchMessage.classList.add("d-none"); // Hide initial message

    const filteredBooks = books.filter(book => book && book.id && !userBookIds.includes(book.id));

    if (filteredBooks.length === 0) {
        const message = books.length === 0 ? "No books found. Try a different search term." : "All found books are already in your lists!";
        DOM.searchResultsDiv.innerHTML = `<p class="initial-message text-muted text-center w-100 p-3">${message}</p>`;
        return;
    }

    filteredBooks.forEach((book) => {
        DOM.searchResultsDiv.appendChild(createBookCard(book, false, null, onAddToList, onViewDetails));
    });
};
...
export const renderUserList = (listElement, books, listName, onViewDetails) => {
    ...
    books.forEach((book) => {
        ...
        listElement.appendChild(createBookCard(book, true, listName, null, onViewDetails));
    });
};
 ...
 */
export const showBookDetailsModal = (book, currentListName, onMoveToList, onRemoveFromList) => {
    ...
    const categories = book.volumeInfo.categories ? book.volumeInfo.categories.join(", ") : "N/A";
    const averageRating = book.volumeInfo.averageRating ? `${book.volumeInfo.averageRating} / 5` : "N/A";
    const ratingsCount = book.volumeInfo.ratingsCount ? `(${book.volumeInfo.ratingsCount} ratings)` : "";
    const googleBooksLink = book.volumeInfo.infoLink || "#";
    const previewLink = book.volumeInfo.previewLink || "#";

    const googleCover = book.volumeInfo.imageLinks?.large || book.volumeInfo.imageLinks?.thumbnail;
    const openLibraryCover = getOpenLibraryCover(book.volumeInfo, 'L'); // Large cover for modal

    const coverImage = googleCover || openLibraryCover || "https://via.placeholder.com/200x300?text=No+Cover";
    ...
    DOM.modalBody.innerHTML = `
        <div class="row align-items-start">
            <div class="col-md-4 text-center mb-3 mb-md-0">
                <img src="${coverImage}" alt="Cover for ${book.volumeInfo.title}" class="cover-image img-fluid rounded mb-3">
                ...
                <div class="d-flex flex-column gap-2">
                    ...
                    ${previewLink !== "#" ? `<a href="${previewLink}" target="_blank" class="btn btn-outline-info btn-sm" rel="noopener noreferrer">Preview Book</a>` : ''}
                </div>
            </div>
    ...
    const lists = [
        { name: "currentlyReading", label: "Currently Reading", btnClass: "btn-success" },
        { name: "wantToRead", label: "Want to Read", btnClass: "btn-info text-white" },
        { name: "readBooks", label: "Read Books", btnClass: "btn-primary" },
    ];

    // Add "Move to" buttons (or "Add to" if not in any list)
    let addedToListAction = false; // Flag to check if book is being added for the first time
    if (!currentListName) {
    ...
    if (currentListName) {
        ...
        removeButton.textContent = `Remove from ${currentListName.charAt(0).toUpperCase() + currentListName.slice(1).replace('Books', ' Books')}`; // Correct 'readBooks' to 'Read Books'
 ...
 * @param {Function} onViewDetails - Callback for viewing book details.
 */
export const displayRecommendations = (recommendedBooks, userBookIds, onAddToList, onViewDetails) => {
    DOM.recommendationsList.innerHTML = "";
    if (DOM.initialRecommendationsMessage) DOM.initialRecommendationsMessage.classList.add("d-none");

    const filteredRecommendations = recommendedBooks.filter(book => book && book.id && !userBookIds.includes(book.id));

    if (filteredRecommendations.length === 0) {
        DOM.recommendationsList.innerHTML = `<p class="initial-message text-muted text-center w-100 p-3">No recommendations found. Read more books to get better suggestions!</p>`;
        if (recommendedBooks.length > 0) { // If there were recommendations but all are already in lists
             DOM.recommendationsList.innerHTML = `<p class="initial-message text-muted text-center w-100 p-3">All recommended books are already in your lists!</p>`;
        }
        return;
    }
    ...
    filteredRecommendations.forEach((book) => {
        ...
        DOM.recommendationsList.appendChild(createBookCard(book, false, null, onAddToList, onViewDetails));
    });
};
...
export const showInitialSearchMessage = (show) => {
    if (DOM.initialSearchMessage) {
        if (show) {
            ...
            DOM.searchResultsDiv.classList.add("d-none"); // Ensure search results are hidden when message is shown
        } else {
...
export const updateAllUserListsUI = (userBooksData, onViewDetails) => {
    renderUserList(DOM.currentlyReadingList, userBooksData.currentlyReading, 'currentlyReading', onViewDetails);
    renderUserList(DOM.wantToReadList, userBooksData.wantToRead, 'wantToRead', onViewDetails);
    renderUserList(DOM.readBooksList, userBooksData.readBooks, 'readBooks', onViewDetails);
};